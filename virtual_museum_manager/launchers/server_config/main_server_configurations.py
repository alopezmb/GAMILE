...
from launchers.server_config.museum_rest_api import api_handlers
...
class MuseumServer(tornado.web.Application):
    ...
    async def _launch_modular_server(self, port=None, open_browser=True, autoreload=False):
        ...
        if port is not None:
            self.port = port
        url = "http://0.0.0.0:{PORT}".format(PORT=self.port)
        print("Test visualization server starting at {url}".format(url=url))
